...
export class Service {
  ...
  constructor() {
    if (config.appwriteUrl && config.appwriteProjectId) {
      ...
      console.warn(
        'Appwrite Service: VITE_APPWRITE_URL or VITE_APPWRITE_PROJECT_ID is not configured in .env'
      );
    }
  ...
  async getPosts(queries = [Query.equal('status', 'active')]) {
    try {
      ...
      if (queries && queries.length > 0) {
        console.log(
          'Appwrite service :: getPosts :: Attempting fallback fetch without queries'
        );
        try {
          const response = await this.databases.listDocuments(
            config.appwriteDatabaseId,
